...
import { AvielIcon } from "@/components/logo";
    ...
    <div className="flex min-h-dvh items-center justify-center bg-[var(--bg-primary)] px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center gap-2 text-center">
          <AvielIcon size={48} priority />
          <h1 className="text-xl font-semibold">Aviel Admin</h1>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {error && (
            <p className="rounded-lg border border-aviel-red-500/30 bg-aviel-red-500/10 px-3 py-2 text-sm text-aviel-red-500">
              {error}
            </p>
          )}
        ...
        <p className="mt-6 text-center text-xs text-[var(--text-secondary)]">
          This is separate from your Aviel account sign-in.
        </p>
      </div>
    </div>
  );
