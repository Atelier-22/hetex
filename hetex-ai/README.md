...
The Next.js frontend for Hetex AI, deployed on Vercel. It renders the UI and
nothing else: no database, no API keys, no model calls. Everything goes through
[`hetex-api`](../hetex-api/README.md).

## What's here

- Email/password auth via NextAuth, verified against the Hetex API
...
- System / light / dark theme with the Hetex green-blue identity
...
The API has to be running first — see [hetex-api](../hetex-api/README.md).
...
Vercel, with **Root Directory set to `hetex-ai`**. Full walkthrough in
