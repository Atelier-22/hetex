...
import { Send, Square, Sparkles, Mic, MicOff, Trash2, RotateCcw, X, Globe, FileText } from "lucide-react";
          ...
          {(pendingFiles.length > 0 || webSearchEnabled || selectedProject) && (
            <div className="mb-2 flex flex-wrap items-center gap-1.5">
              ...
              {pendingFiles.map((f) => (
                ...
                >
                  {f.previewUrl ? (
                    <img src={f.previewUrl} alt="" className="h-5 w-5 rounded-full object-cover" />
                  ) : (
                    <FileText size={13} className="text-[var(--text-secondary)]" />
                  )}
                  <span className="max-w-[120px] truncate">{f.name}</span>
                  <button onClick={() => removeFile(f.name)} aria-label="Remove attachment">
