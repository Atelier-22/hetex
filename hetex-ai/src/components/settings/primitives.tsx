    ...
    <div className={`flex flex-col gap-1 ${className}`}>
      <input
        ...
        className={`focus-ring w-full rounded-lg border bg-[var(--bg-secondary)] px-3 py-2 text-sm outline-none disabled:cursor-not-allowed disabled:opacity-40 ${
          error ? "border-hetex-red-500" : "border-[var(--border-subtle)]"
        }`}
      />
      {error && <span className="text-xs text-hetex-red-500">{error}</span>}
    </div>
  );
}
...
export function SaveIndicator({
  ...
  if (state === "error") {
    return (
      <span
        role="alert"
        className="flex items-center gap-2 text-xs text-hetex-red-500"
      >
...
export function StatusPill({
  ...
  const styles: Record<PillTone, string> = {
    ...
    off: "border-transparent bg-hetex-red-500/12 text-hetex-red-500",
